/*
  ...
  1. Schema Changes
    - Add event_type column to events table with check constraint
    - Update all RPC functions to include event_type
...
DROP FUNCTION IF EXISTS create_event_with_reminders(uuid, text, timestamptz, text, timestamptz, text, uuid[], boolean, text, text);
DROP FUNCTION IF EXISTS update_event_with_reminders(uuid, text, text, timestamptz, timestamptz, text, uuid[], boolean, text, text);
