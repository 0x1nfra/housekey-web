        ...
        .on(
          ...
          (payload: RealtimePostgresChangesPayload<Event>) => {
            try {
              ...
              set((state: EventsState) => {
                state.error =
                  "Failed to process event update. Please refresh the page.";
              });
            }
          }
        )
              ...
              set((state: EventsState) => {
                if (payload.eventType === "INSERT") {
                  ...
                  state.reminders = state.reminders.filter(
                    (r) => r.id !== deletedReminder.id
                  );
                }
              });
            } catch (error) {
              console.error(
                "Error handling reminder subscription update:",
                error
              );
              set((state: EventsState) => {
                state.error =
                  "Failed to process reminder update. Please refresh the page.";
              });
            }
          }
        )
      ...
      const subscriptionGroup: SubscriptionGroup = {
        ...
        unsubscribe: () => {
          try {
            ...
            console.error(
              "Error unsubscribing from event subscriptions:",
              error
            );
          }
        },
