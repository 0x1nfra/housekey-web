...
import { HubStore } from "../hub/types";
import { createHubSelectors } from "../hub/selectors";
...
export const createShoppingSelectors = (
  state: ShoppingState,
  hubState: HubStore
) => {
  const hubSelectors = createHubSelectors(hubState);
  return {
    // List selectors
    ...
    canUserManage: () => {
      return hubSelectors.isCurrentUserManagerOrOwner();
    },
    ...
    getShoppingSuggestions: (hubId: string) => {
      ...
      const itemFrequency: Record<
        string,
        { count: number; category?: string; icon?: string }
      > = {};
