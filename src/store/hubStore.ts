...
export const useHubStore = create<HubStore>((set, get) => ({
  ...
  createHub: async (data: CreateHubData) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to create hub';
  ...
  updateHub: async (hubId: string, data: UpdateHubData) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to update hub';
      set({ error: errorMessage, loading: false });
      return { success: false, error: errorMessage };
    }
  },
  ...
  deleteHub: async (hubId: string) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to delete hub';
      set({ error: errorMessage, loading: false });
      return { success: false, error: errorMessage };
    }
  },
  ...
  inviteMember: async (hubId: string, data: InviteMemberData) => {
    ...
    try {
      ...
      const { data: existingMember } = await supabase
        ...
        .select('id')
        .eq('hub_id', hubId)
        .eq('user_id', (await supabase.from('user_profiles').select('id').eq('email', data.email).single()).data?.id)
        .single();
  ...
  cancelInvitation: async (invitationId: string) => {
    ...
    try {
      ...
      const updatedInvites = pendingInvites.filter(i => i.id !== invitationId);
    ...
    try {
      ...
      const { data: invites, error: invitesError } = await supabase
        ...
        .select(`
          *,
          hub:hubs(name),
          invited_by_profile:user_profiles!hub_invitations_invited_by_fkey(name)
        `)
        .eq('hub_id', hubId)
        .is('accepted_at', null);
  ...
  getHubPermissions: (hubId?: string): HubPermissions => {
    const { currentHub, hubMembers } = get();
    ...
    const currentUser = hubMembers.find(m => 
      m.hub_id === targetHubId && 
      m.user_id === (supabase.auth.getUser() as any)?.data?.user?.id
    );
  ...
  subscribeToHubChanges: (hubId: string) => {
    ...
    const invitesSubscription = supabase
      ...
      .on('postgres_changes',
        { event: '*', schema: 'public', table: 'hub_invitations', filter: `hub_id=eq.${hubId}` },
        () => {
          get().loadHubData(hubId);
        }
      )
