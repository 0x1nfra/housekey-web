/*
  # Create Calendar RPC Functions

  1. New Functions
    - `get_calendar_data` - Returns combined events and tasks for calendar view with filtering
    - `get_hub_events_with_attendees` - Returns events with attendee details
    - `get_upcoming_events` - Returns upcoming events for a hub
    - `create_event_with_reminders` - Creates event with optional reminders
    - `update_event_with_reminders` - Updates event with optional reminders  
    - `delete_event_with_reminders` - Deletes event and associated reminders

  2. Security
    - All functions respect existing RLS policies
    - Functions only return data user has access to

  3. Features
    - Combined calendar view of events and tasks
    - Filtering by data type, assigned user, and event type
    - Attendee management for events
    - Reminder management for events
*/
